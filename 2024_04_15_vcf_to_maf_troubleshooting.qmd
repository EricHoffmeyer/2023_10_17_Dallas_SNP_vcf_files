...
Now we can download and install ANNOVAR. You need to enter your name and e-mail on the [ANNOVAR website](https://www.openbioinformatics.org/annovar/annovar_download_form.php) to get the zip file. After that, we need to download and install extra libraries.
...
It's not mentioned on the website, but the `annotate_variation.pl` script has the download function. The command syntax is:

`perl annotate_variation.pl -buildver (genome version) -downdb (filename) (path)`
...
# perl annotate_variation.pl -buildver hg19 -downdb exac03 humandb/
# perl annotate_variation.pl -buildver hg19 -downdb avsnp147 humandb/
# perl annotate_variation.pl -buildver hg19 -downdb dbnsfp30a humandb/
```

NOTE: These files are about 10-20GB total, so the download will take some time. When they finish, unzip and remove the contents.

## table_annovar.pl 

Now we can run `table_annovar.pl`. The command syntax is:

perl table_annovar (file path) (library path) -buildver (genome version) --out (result path)
